import React, { useState, useEffect } from 'react';
import { hot } from 'react-hot-loader/root';
import classNames from 'classnames/bind';

const style = require('./country.scss');
const cx = classNames.bind(style);

const data = {
  activeCountry: 'Keongi',
  country: [
    {
      id: 'Keongi',
      title: '건지',
      image: {
        src: 'data:image/png;base64,'
      }
    },
    {
      id: 'Greece',
      title: '그리스',
      image: {
        src: 'http://www.0404.go.kr/imgsrc.mofa?atch_file_id=FILE_000000000010504&file_sn=1'
      }
    }
  ]
};

const Country = () => {
  const [activeCountry, setActiveCountry] = useState('Keongi');

  useEffect(() => {
    console.log('mount');
  }, []);

  useEffect(() => {
    console.log('activeCountry:', activeCountry);

    return () => {
      console.log('prevState:', activeCountry);
    };
  });

  return (
    <div className={cx('country')}>
      <button className={cx('continent')} aria-selected="false">유럽</button>
      <ul className={cx('country_set')} role="radiogroup">
        {
          data.country.map(item => (
            <li className={cx('list')} role="presentation" key={item.id}>
              <img className={cx('country_image')} src={item.image.src} alt={item.title}/>
              <label htmlFor={item.id} className={cx('country_title')}>{item.title}</label>
              <input type="radio" id={item.id} name="country" className={cx('country_input')} aria-checked={activeCountry === item.id} onClick={() => { setActiveCountry(item.id); }}/>
            </li>
          ))
        }
      </ul>
    </div>
  );
};

export default hot(Country);
